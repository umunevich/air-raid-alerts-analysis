...
| **Alert scope** | Any active air raid alert (type-agnostic; see [Alert types](#alert-types-deferred)) |
...
Let `A_r(τ)` be 1 if region `r` is **under an active air raid alert** at instant `τ`, and 0 otherwise. An alert is active from `started_at` until `finished_at` (or until censored; see [Data requirements](#data-requirements)).
...
## Alert types (deferred)

The MVP treats all alerts as a **single binary state**: region is either under an air raid alert or not. Alert types (e.g. air, drone, ballistic, artillery) are **not modeled, stored, or shown** in the current phase.

**Rationale for deferring:** The primary dataset (Vadimkin official CSV) has no `alert_type` column. Type enrichment from secondary sources adds pipeline complexity without blocking a first exposure model.

**Future phase (when requirements are updated):** Per-type tracking, internal sub-models, and optional enrichment rules are described in [DATA.md](./DATA.md#future-alert-types).
...
| `finished_at` | When the alert cleared (UTC); nullable if still open at ingest time |

### Derived series (hourly, per region)

At each hour `t` and region `r`:

- `active(r, t)` — 1 if the region is under an alert during that hour (or at hour boundary; define consistently in code).
- Optional: minutes under alert within the hour, time since last start/end.

### Label construction

From `active` and event intervals, build `y_r,N(t)` for all `N ∈ 1..24` using the exposure definition above. Unit tests must assert **no future information** in features at `t`.

### Data quality

- Deduplicate repeated records for the same `alert_id`.
- Normalize timezones to UTC at ingest.
- **Censoring:** alerts without `finished_at` at dataset cutoff — document handling (e.g. exclude open-ended intervals from duration stats; treat as active until ingest time for label construction, with a noted bias).
...
2. **Seasonal baseline:** same, stratified by hour-of-week.
3. **Persistence:** if `active(r, t) = 1`, predict high `P` for short N; else low.

### Primary models (implementation choice)

- Single **binary exposure model** on `y_r,N(t)` (logistic regression, gradient boosting, or similar).
...
- **Alert type** differentiation (modeling, features, UI, or enrichment)
...
| **User-facing probability** | `P_r,N(t)` — one number per horizon N. |
...
| 2026-06-20 | Defer alert types; MVP is type-agnostic binary exposure model. |
